...
export const SummaryPage = () => {
  ...
  const skills = useMemo(() => filterVisibleItems<Translation>('skills'), [data?.skills])

  const formation = useMemo(() => filterVisibleItems<Translation>('formation'), [data?.formation])

  const languages = useMemo(() => filterVisibleItems<Translation>('languages'), [data?.languages])

  const sectors = useMemo(() => filterVisibleItems<Translation>('sectors'), [data?.sectors])
      ...
      {data && (
        <Stack direction={'row'} flexWrap={'wrap'} mt={1} gap={2}>
          ...
          >
            ...
            <Typography fontFamily={'BentonSans'}>{data.description.fr}</Typography>
          </Card>
