...
import { ResumeSection, Translation, UserDataExperience } from '@/core/storage/types/storage'
          ...
          <Stack gap={2} sx={{ minWidth: '280px', flex: 1 }}>
            {(showSectors || showSkills) && (
              <Card size="lg" sx={{ flex: 1 }}>
                ...
                <List sx={{ p: 0, gap: 0.5 }}>
                  {skills.map((skill) => (
                    <Typography key={skill.id} fontFamily={'BentonSans'} component={'li'}>
                      {skill.content.fr}
                    </Typography>
                  ))}
                  ...
                  {sectors.map((sector) => (
                    <Typography key={sector.id} fontFamily={'BentonSans'} component={'li'}>
                      {sector.content.fr}
                    </Typography>
                  ))}
                </List>
              ...
              <Card size="lg" sx={{ minWidth: '200px', flex: 1 }}>
                ...
                <List sx={{ p: 0 }}>
                  {languages.map((language) => (
                    <Typography key={language.id} fontFamily={'BentonSans'} component={'li'}>
                      {language.content.fr}
                    </Typography>
                  ))}
                </List>
          ...
          <Stack direction={'row'} flexWrap={'wrap'} gap={2} sx={{ minWidth: '280px', flex: 1 }}>
            {showFormation && (
              <Card size="lg" sx={{ minWidth: '200px', minHeight: '140px', flex: 1 }}>
                ...
                <List sx={{ p: 0 }}>
                  {formation.map((el) => (
                    <Typography key={el.id} fontFamily={'BentonSans'} component={'li'}>
                      {el.content.fr}
                    </Typography>
                  ))}
                </List>
            ...
            {showEmploymentHistory && (
              <Card size="lg" sx={{ minWidth: '200px', minHeight: '140px', flex: 2 }}>
                ...
                {employmentHistory.map((employment) => (
                  <Typography key={employment.id} fontFamily={'BentonSans'}>
